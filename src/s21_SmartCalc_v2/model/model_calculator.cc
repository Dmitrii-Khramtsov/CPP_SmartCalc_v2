...
Vector ModelCalculator::calculateGraf(std::pair<double, double> xRange, std::pair<double, double> yRange,
    ...
    if ((xRange.second < xRange.first) || (yRange.second < yRange.first)) {
        throw std::invalid_argument("Не коректно введены граници отображения графика");
    }
    ...
    if (vXYOutPut[0].empty()) {
        throw std::invalid_argument("ни одна из точек не находится в заданной области значений");
    }
    return vXYOutPut;
}
...
bool ModelCalculator::isUnaryOperator(const char u_op) {
    static const std::set<char> unary_operators = {'s', 'c', 't', 'i', 'o', 'n', 'q', 'l', 'g', '~'};
 ...
 */
double ModelCalculator::applyBinaryOperator(const char b_op, double a, double b) {
    switch (b_op) {
        case '+':
            return a + b;
        case '-':
...
double ModelCalculator::applyUnaryOperator(const char u_op, double a) {
    switch (u_op) {
        ...
        case 'q':
            if (a < 0.0) {
                throw std::invalid_argument("The expression under the root cannot be negative");
            }
            return std::sqrt(a);
        case 'l':
            if (a <= 0.0) {
                throw std::invalid_argument("The expression under the logarithm cannot be zero or negative.");
            }
            return std::log(a);
        case 'g':
            if (a <= 0.0) {
